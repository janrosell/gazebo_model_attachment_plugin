...
namespace gazebo
{
    ...
    void ModelAttachmentPlugin::Load(physics::WorldPtr world, sdf::ElementPtr sdf)
    {
        ...
        attach_srv_ = node_->create_service<gazebo_model_attachment_plugin::srv::Attach>("/gazebo/attach", std::bind(&ModelAttachmentPlugin::attachCallback, this, std::placeholders::_1, std::placeholders::_2));
        detach_srv_ = node_->create_service<gazebo_model_attachment_plugin::srv::Detach>("/gazebo/detach", std::bind(&ModelAttachmentPlugin::detachCallback, this, std::placeholders::_1, std::placeholders::_2));
    }

    bool ModelAttachmentPlugin::attachCallback(const std::shared_ptr<gazebo_model_attachment_plugin::srv::Attach::Request> req,
                                               std::shared_ptr<gazebo_model_attachment_plugin::srv::Attach::Response> res)
    {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Received request to attach model: '" << req->model_name_1 << "' to '" << req->model_name_2);
    ...
    bool ModelAttachmentPlugin::detachCallback(const std::shared_ptr<gazebo_model_attachment_plugin::srv::Detach::Request> req,
                                               std::shared_ptr<gazebo_model_attachment_plugin::srv::Detach::Response> res)
    {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("rclcpp"), "Received request to detach model: '" << req->model_name_1 << "' from '" << req->model_name_2);
